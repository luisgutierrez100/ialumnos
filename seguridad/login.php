...
if (isset($_GET['a'])) {
	...
	if($_GET['a']=='A'){$tipo=1;}else{$tipo=2;}
     
}else{
	//echo "variable no enviada redireccionar";
...
if (isset($_POST['ci'])) {
  ...
  mysqli_select_db( $conecta,$database_conecta) or die(mysqli_error($conecta));
  
  $LoginRS__query="SELECT CiAlumno, Email FROM alumnos WHERE CiAlumno=$loginUsername AND Email='$password'";
   $LoginRS = mysqli_query($conecta,$LoginRS__query) or die(mysqli_error($conecta));
...
<div class="login">
  ...
  <div class="login-form">
    ...
    <input hidden="true" name="tipo" value="alumno" />
    <input type="text" id="password" placeholder="info@ibnorca.org" name="password"/>
    <br>
...
header("location: docentelogin2.php");
