...
from ibnorca.docente a where a.CiDocente=$ci";
...
<form class="contact_form" onSubmit="return validar_clave()" action="Inf_recD.php" method="post" name="form1">
 <div>
   <ul>
   ...
   <span class="required_notification">* Datos requeridos</span> </li>
     ...
     <li>
     <label for="nempresa">Da Factura?:</label>
    <select name="factura">
       
        <option value="1" <?php if ($fact==1) {?> selected <?php } ?> >Si </option>
        <option value="0" <?php if ($fact==0) {?> selected <?php } ?>>No</option>
     ...
     <li>
     <label for="fempresa">Teléfono Empresa:</label><input type="text" name="TEmpresa" id="TEmpresa" onkeypress="return valida(event)" value="<?php if(isset($row6['FonoOficina'])){echo $row6['FonoOficina'];}?>" />
     </li>
